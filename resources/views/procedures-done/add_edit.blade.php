...
@section('content')
    ...
    <div class=" container ">
        @if ($action == 'add')
            ...
            $form_action = '/'.env('ADMIN_URL').'/procedures-done';
            ?>
        @else
            <?php 
            $title = 'Edit';
            ...
            $form_action = '/'.env('ADMIN_URL').'/procedures-done/'.$item->id;
            ?>
        @endif
        <div class="card card-custom">
            ...
            <form class="form" method="POST" action="{{$form_action}}">
                ...
                <div class="card-body">
                    ...
                    {{-- <div class="form-group">
                        <label for="kt_select2_1">Visit</label>
                        <select class="form-control select2" id="kt_select2_2" name="procedure_type_id" >
                            <option value="0">Select Visit</option>
                            @foreach($visits as $visit)
                                <option {{ $visit->id == $item_visit_id ? 'selected' : '' }} value="{{ $visit->id }}" >{{$visit->name}}</option>
                            @endforeach
                        </select>
                    </div> --}}
